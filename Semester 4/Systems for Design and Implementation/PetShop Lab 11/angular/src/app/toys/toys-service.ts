...
export class ToysService {
  ...
  getToysOfCat(customerId: number): Observable<ToysDTO> {
    return this.http.get<ToysDTO>(this.backendUrl + `/${customerId}`);
  }
  ...
  addToy(name: string, price: number, cat: Cat): Observable<any> {
    return this.http.post(this.backendUrl, new Toy(name, price, cat));
  }
