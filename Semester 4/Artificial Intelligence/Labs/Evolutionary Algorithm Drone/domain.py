...
from utils import DIRECTIONS, MAP_LENGTH
...
class Individual:
    def __init__(self, drone_map, battery, size=0, chromosome=None):
    ...
    def update_fitness(self):
        ...
        for x in self.chromosome:
            marked[drone[0]][drone[1]] = 1
            for direction in DIRECTIONS:
                sight = deepcopy(drone)
                while True:
                    sight[0] += direction[0]
                    sight[1] += direction[1]
                    valid = False
    ...
    @staticmethod
    def crossover(drone_map, first_parent, other_parent, crossover_probability=0.9):  # 1-cutting point crossover
        ...
        if random() < crossover_probability:
            ...
            offspring1, offspring2 = Individual(drone_map, first_parent.battery, size), Individual(drone_map,
...
class Population:
    def __init__(self, drone_map, battery=10, population_size=10, individual_size=10, population=None):
        self.population_size = population_size
        if population is None:
            population = [Individual(drone_map, battery, individual_size) for _ in range(population_size)]
